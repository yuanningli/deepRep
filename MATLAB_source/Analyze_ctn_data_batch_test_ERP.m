...
for i = 1 : length(task_list)
    ...
    for j = 1 : num_sessions(i)
        ...
        time_onset = cat(2, time_onset, round(temp.Time_onsets/2));
    end
end
...
N = size(data_test,2);
num_trials = length(temp.Time_onsets); % 200 ms trials with 50ms step size
batch_size = length(temp.Time_onsets);
...
while batch_cnt < num_batches
    ...
    csvwrite(['Test_data_normed_ERP_',task_list{1},'_batch',num2str(batch_cnt),'.dat'],batch_data)
    csvwrite(['Test_data_ERP_',task_list{1},'_label_batch',num2str(batch_cnt),'.dat'],batch_label)
